...
pacman::p_load(knitr, tidyverse, dplyr, lubridate, rvest, dataRetrieval, sf, ggplot2, RColorBrewer, shiny, measurements, leaflet, mapview, has_crop_tools, kable, ggpmisc, ggpubr)
...
The first step to explore the changes in the discharge of the Neus River downstream of the Falls Lake Dam is to conduct a time series analysis. From the time series analysis results (Figure 3-6), all the 4 gages show clear seasonal cycles, but the general trends remain vague. 
...
To further explore the trend of discharge over time, we performed a linear regression analysis on the data (Figure 7-10). Discharge at Falls Lake and Clayton gage decreased with a negative slope, while the slopes of Goldsboro and Kinston are slightly positive. However, the R square of all the 4 regrssion are small, which means the fitting is not good for the flow data with large fluctuation. 
...
、
The number of Dams being removed in North Carolina has increased in recent years, and the hydrological impacts of dams on rivers remains unclear. Taking the Falls Lake Dam for a closer study on its hylogical impacts, we analysed the changes of downstream dicharge before and after its consstruction. After the construction of the Falls Lake Dam, the discharge at the 2 downstream gages closest to the dam (Falls Lake and Clayton) decreased, while the discharge of 2 farther gages (Goldsboro and Kinston) had no obvious change. It may be because longer distance and annexed tributaries weaken the dam's influence on the discharge at these 2 gages. We can conclude that the Falls Lake Dam reduced the discharge of a river within a certain distance. This information is valuable for the hydrological effects of other dams in North Carolina, as well as for dam management and future dam removal.
